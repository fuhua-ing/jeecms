...
import com.jeecms.common.email.EmailConstants;
import com.jeecms.common.email.EmailSender;
import com.jeecms.common.email.MessageTemplate;
import com.jeecms.common.email.SendEmailService;
...
public class CmsUserMngImpl implements CmsUserMng
{
	...
	public Pagination getPage(String username, String email, Integer siteId, Integer groupId, Boolean disabled,
			Boolean admin, Integer rank, String realName, Integer roleId, Boolean allChannel, int pageNo, int pageSize)
	{
		Pagination page = dao.getPage(username, email, siteId, groupId, disabled, admin, rank, realName, roleId,
	...
	private CmsUser updateAdmin(CmsUser bean, CmsUserExt ext, String password, Integer groupId, Integer[] roleIds,
			Integer[] channelIds)
	{
	...
	@Override
	public Boolean sendEmail(String username, String changeType)
	{
		boolean f = false;
		try
		{
			CmsUser cmsUser = dao.findByUsername(username);
			JcEvent jcEvent = jcEventDao.getByIdAndType(cmsUser.getId(), changeType);
			jcEvent.setEventUserId(cmsUser.getId());
			...
			if (ChangeEunm.Email.getValue().equals(changeType))
			{
				subject = EmailConstants.CHANGE_EMAIL_SUBJECT;
				context = StringUtils.replace(EmailConstants.CHANGE_EMAIL, "${key}", uuid);
				jcEvent.setEventType(ChangeEunm.Email.getValue());
			}
			else
			{
				subject = EmailConstants.CHANGE_CELL_PHONE_SUBJECT;
				context = StringUtils.replace(EmailConstants.CHANGE_CELL_PHONE, "${key}", uuid);
				jcEvent.setEventType(ChangeEunm.Phone.getValue());
			}
			...
			jcEvent.setEventApplyTime(eventApplyTime);
			jcEvent.setEventEndTime(endTimes);

			jcEvent.setEventUuid(uuid);

			sendEmail.send(subject, context, cmsUser.getEmail());
			jcEventDao.saveEntity(jcEvent);
			f = true;
			return f;
		}
		catch (Exception e)

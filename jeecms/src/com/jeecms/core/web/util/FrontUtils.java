...
public class FrontUtils
{
			...
			Map<String, Object> model)
	{
		...
		return getTplPath(request, site.getSolutionPath(), TPLDIR_COMMON, PAGE_NOT_FOUND);
	}
	...
	public static String showSuccess(HttpServletRequest request, Map<String, Object> model, String nextUrl)
	{
		...
		return getTplPath(request, site.getSolutionPath(), TPLDIR_COMMON, SUCCESS_PAGE);
	}
	 ...
	 * @return
	 */
	public static String showError(HttpServletRequest request, HttpServletResponse response, Map<String, Object> model,
			WebErrors errors)
	{
		CmsSite site = CmsUtils.getSite(request);
		frontData(request, model, site);
	 ...
	 * @return
	 */
	public static String showMessage(HttpServletRequest request, Map<String, Object> model, String message,
			String... args)
	{
		CmsSite site = CmsUtils.getSite(request);
		frontData(request, model, site);
	...
	public static void frontData(Map<String, Object> map, CmsSite site, CmsUser user, String location, Long startTime)
	{
		...
		String mobileRes = ctx + RES_PATH + "/" + site.getPath() + "/" + site.getTplMobileSolution();
		// res路径需要去除第一个字符'/'
			...
			throws TemplateException, IOException
	{
		...
		{
			String tpl = getTplPath(site.getSolutionPath(), TPLDIR_STYLE_LIST, userPage);
			env.include(tpl, UTF8, true);
		}
		else
		{
			...
			throws IOException, TemplateException
	{
		...
		{
			tpl = getTplPath(site.getSolutionPath(), TPLDIR_TAG, tplName + "_" + subTpl);
		}
		env.include(tpl, UTF8, true);
	}
	 ...
	 * @throws TemplateException
	 */
	public static void includeTpl(String listStyle, CmsSite site, Environment env) throws IOException,
			TemplateException
	{
		String tpl = getTplPath(site.getSolutionPath(), TPLDIR_STYLE_LIST, listStyle);
		env.include(tpl, UTF8, true);
	}

	public static String newShowError(HttpServletRequest request, HttpServletResponse response, ModelMap model)
	{
		CmsSite site = CmsUtils.getSite(request);
		frontData(request, model, site);
		return getTplPath(request, site.getSolutionPath(), TPLDIR_COMMON, ERROR_PAGE);
